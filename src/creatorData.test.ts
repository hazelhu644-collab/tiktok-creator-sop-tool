...
describe("editable creator data helpers", () => {
  it("updates edited creator fields and normalizes safe video progress text for export", () => {
    ...
    expect(
      updateCreatorField(row(), "lastFollowUpCount", "3").lastFollowUpCount,
    ).toBe(3);
    expect(
      updateCreatorField(row(), "lastFollowUpCount", "").lastFollowUpCount,
    ).toBe(0);
  });
  ...
  it("preserves archived collaboration history and stable identity in localStorage", () => {
    ...
    expect(loadCreatorRows()).toContainEqual(
  ...
  it("exports the current rows with UTF-8 BOM, Chinese headers, and compact tracking counts", () => {
    ...
    expect(csv.split("\n")[0]).toBe(
      "\ufeff达人账号,主页链接,联系渠道,店铺 / 品牌,产品,合作状态,样品到货日期,视频进度,首条视频发布日期,最近联系日期,跟进次数,跟进状态,最近沟通动作,最近沟通渠道,下次跟进日期,达人回复,达人备注,是否同达人多样品,同达人样品数量",
    );
  ...
  it("allows same creator in the same store with a different product as valid multi-sample creator", () => {
    ...
    const secondSample = row({
      id: "b",
      profileLink: "https://www.tiktok.com/@creatorA",
  ...
  it("summarizes imported possible duplicates and valid multi-sample records without deleting rows", () => {
    const existing = [
      row({ id: "existing", username: "@creatorA", product: "Pet Brush" }),
    ];
    const incoming = [
  ...
  it("migrates existing rows without store fields to 默认店铺", () => {
    window.localStorage.setItem(
      CREATOR_ROWS_STORAGE_KEY,
      JSON.stringify([{ ...row(), storeId: undefined, storeName: undefined }]),
    );
  ...
  it("imports Store and Brand headers into storeName and exports 店铺 / 品牌", () => {
    const importedStore = normalizeRecord(
      { Store: "Shop A", "Creator username": "store_creator" },
      0,
    );
    const importedBrand = normalizeRecord(
      { Brand: "PinePaw", "Creator username": "brand_creator" },
      1,
    );
  ...
  it("imports ETA and estimated arrival headers into the single sample arrival date field", () => {
    expect(
      normalizeRecord(
        { ETA: "2026-06-12", "Creator username": "eta_creator" },
        0,
      ).sampleDeliveredDate,
    ).toBe("2026-06-12");
    expect(
      normalizeRecord(
        {
          ...
          "Creator username": "arrival_creator",
        },
        1,
      ).sampleDeliveredDate,
    ).toBe("2026-06-13");
