...
export function createCampaignFromName(name: string, fallback: CreatorFilmingRequirements = defaultCreatorFilmingRequirements): Campaign {
  ...
  return {
    ...
    videoCount: preset.videoCount ?? String((preset.requirements ?? fallback.requirements).join('\n').match(/(\d+)\s*条视频/)?.[1] ?? '2'),
    videoLength: preset.videoLength ?? ((preset.requirements ?? fallback.requirements).find((item) => item.includes('秒')) ?? ''),
    tagRequirement: preset.tagRequirement ?? '必须挂 TikTok Shop 产品链接',
...
export function campaignToFilmingRequirements(campaign: Campaign | undefined, fallback: CreatorFilmingRequirements): CreatorFilmingRequirements {
  ...
  return {
    ...
    requirements: campaign.requirements?.length ? campaign.requirements : fallback.requirements,
    keyContentPoints: campaign.keyContentPoints?.length ? campaign.keyContentPoints : fallback.keyContentPoints,
    referenceLinks: campaign.referenceLinks ?? fallback.referenceLinks,
  };
}
