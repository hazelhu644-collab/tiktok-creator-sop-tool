...
export function normalizeVideoProgress(value: unknown): VideoProgressNormalization {
  if (value instanceof Date) return { normalized: '', warning: VIDEO_PROGRESS_WARNING };
...
function progressFor(row: CreatorRow): string {
  return normalizeVideoProgress(row.videoProgress).normalized;
}

function isCompleted(row: CreatorRow): boolean {
  return normalizeText(row.currentStatus).toLowerCase() === 'completed' || progressFor(row) === '2/2';
}
