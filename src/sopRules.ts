...
export function parseRequiredVideos(
  source: FilmingRequirementsLike | string[] | string | null | undefined,
): number {
...
function requiredVideoExamples(requiredVideos: number): string[] {
  return Array.from(
    { length: requiredVideos + 1 },
    (_, postedCount) => `${postedCount} of ${requiredVideos}`,
  );
}

export function buildVideoProgressHint(
  requiredVideos = DEFAULT_REQUIRED_VIDEOS,
): string {
  return `视频进度建议填写 ${requiredVideoExamples(requiredVideos).join("、")}，避免 Excel 自动转成日期。`;
}

export function buildVideoProgressWarning(
  requiredVideos = DEFAULT_REQUIRED_VIDEOS,
): string {
  return `视频进度可能无法识别。建议使用 ${requiredVideoExamples(requiredVideos).join("、")}。`;
}

export const VIDEO_PROGRESS_WARNING = buildVideoProgressWarning(
  DEFAULT_REQUIRED_VIDEOS,
);
...
export function normalizeVideoProgress(
  ...
  const safeRequiredVideos =
    Number.isSafeInteger(requiredVideos) && requiredVideos > 0
      ? requiredVideos
      : DEFAULT_REQUIRED_VIDEOS;
  ...
  if (value instanceof Date) {
    return {
      normalized: "",
      warning: buildVideoProgressWarning(safeRequiredVideos),
      requiredVideos: safeRequiredVideos,
    };
  }
  ...
  if (!raw)
    return {
      normalized: `0/${safeRequiredVideos}`,
      postedCount: 0,
      requiredVideos: safeRequiredVideos,
    };
  ...
  if (ofMatch)
    return normalizePostedCount(
      Number.parseInt(ofMatch[1], 10),
      Number.parseInt(ofMatch[2], 10),
      safeRequiredVideos,
      raw,
    );
  ...
  if (videosMatch)
    return normalizePostedCount(
      Number.parseInt(videosMatch[1], 10),
      safeRequiredVideos,
      safeRequiredVideos,
      raw,
    );
  ...
  if (postedMatch)
    return normalizePostedCount(
      Number.parseInt(postedMatch[1], 10),
      safeRequiredVideos,
      safeRequiredVideos,
      raw,
    );
...
function normalizePostedCount(
  postedCount: number,
  providedRequiredVideos: number,
  requiredVideos: number,
  raw: string,
): VideoProgressNormalization {
  const safeProvidedRequiredVideos =
    Number.isSafeInteger(providedRequiredVideos) && providedRequiredVideos > 0
  ...
  if (!Number.isSafeInteger(postedCount) || postedCount < 0) {
    return {
      normalized: raw,
      warning: buildVideoProgressWarning(requiredVideos),
      requiredVideos,
    };
  }
  ...
  if (postedCount > safeProvidedRequiredVideos) {
    return {
      ...
      postedCount,
      requiredVideos: safeProvidedRequiredVideos,
      isOverRequired: true,
    };
  }
...
function looksLikeExcelConvertedDate(
  value: unknown,
  normalized: string,
): boolean {
...
export function isDeliveredLogisticsStatus(status: string): boolean {
  return includesAny(normalizeText(status), [
    "delivered",
    "已签收",
    "已到货",
    "签收",
...
export function isInTransitLogisticsStatus(status: string): boolean {
  ...
  if (
    !normalized ||
    ["not shipped", "not started", "pending", "未发货", "未寄样"].includes(
      normalized,
    )
  )
    return false;
...
export function arrivalDateDeltaDays(
...
function hasDeliveredEvidence(row: CreatorRow): boolean {
  return (
    ...
    includesAny(normalizeText(row.currentStatus), [
      "delivered",
      "已签收",
      "已到货",
      "签收",
...
function isNoSampleSent(status: string): boolean {
  const normalized = normalizeText(status).toLowerCase();
  return (
    !normalized || normalized === "pending" || normalized === "not shipped"
  );
}
...
function isCompleted(row: CreatorRow, requiredVideos: number): boolean {
  ...
  return (
    normalizeText(row.currentStatus).toLowerCase() === "completed" ||
    (typeof progress.postedCount === "number" &&
      progress.postedCount >= requiredVideos)
  );
}

function missingVideoCount(
  progress: VideoProgressNormalization,
  requiredVideos: number,
): number | null {
...
export function getFailureWarnings(
  row: CreatorRow,
  today = new Date(),
  requiredVideos = DEFAULT_REQUIRED_VIDEOS,
): string[] {
  ...
  if (
    ...
    warnings.push(
      `样品已到货 ${deliveredDays} 天，但视频进度仍为 0/${requiredVideos}。`,
    );
  }
  ...
  if (
    ...
    warnings.push(
      `达人已发布 ${progress.postedCount} 条视频，还差 ${missingVideos} 条视频，首条视频已发布 ${firstVideoDays} 天。`,
    );
  }
  ...
  if (
    ...
    warnings.push(
      "备注显示达人长期未回复、没有明确拍摄计划、合作状态较差，或不愿意修改视频。",
    );
  }
...
function isHandledToday(row: CreatorRow, today: Date): boolean {
  ...
  return (
    isSameDate(row.lastHandledDate, today) ||
    isSameDate(row.lastMessageSentAt, today) ||
    Boolean(
      row.followUpHistory?.some(
        (entry) =>
          isSameDate(entry.date, today) &&
          handledActions.includes(entry.action),
      ),
    )
  );
}
...
function isShippedOrInTransit(row: CreatorRow): boolean {
  return (
    ...
    (isInTransitLogisticsStatus(row.currentStatus) &&
      !hasDeliveredEvidence(row))
  );
}
...
function stageRank(
  ...
  if (
    ["replied", "reply pending", "达人已回复", "达人回复待处理"].includes(
      trackingStatus,
    )
  )
    return 1;
...
export function analyzeCreator(
  ...
  const progress = videoProgress(row, requiredVideos);
  const missingVideos = missingVideoCount(progress, requiredVideos);
  const hasPostedAnyVideo =
    typeof progress.postedCount === "number" && progress.postedCount > 0;
  const isIncomplete = missingVideos === null || missingVideos > 0;
  const trackingStatus = normalizeText(row.trackingStatus).toLowerCase();
  const hasPendingCreatorReply =
    ["replied", "reply pending", "达人已回复", "达人回复待处理"].includes(
      trackingStatus,
    ) &&
  ...
  if (row.archivedAt) {
    ...
    return {
      ...row,
      videoProgress: progressArchived.normalized,
      videoProgressWarning: progressArchived.warning,
      priority: "Low",
      priorityRank: PRIORITY_RANK.Low,
      stageRank: 9,
      triggerReason: "合作记录已归档，默认不进入今日队列。",
      suggestedAction: "可在显示已归档合作后查看或恢复。",
      failedWarnings: [],
      needsFollowUp: false,
    };
  }
  ...
  if (!normalizeText(row.username) || !hasAnyWorkflowSignal) {
    ...
    suggestedAction =
      "生成「回复达人消息」话术，基于达人回复内容给出下一步回应。";
  } else if (
    (pauseNote || futureFollowUp) &&
    !dueFollowUp &&
    !isShippedOrInTransit(row)
  ) {
    priority = "Low";
    triggerReason = pauseNote
      ? "备注显示暂不催，已降低优先级。"
      : "下次跟进日期未到，暂不进入今日高优先级。";
    ...
    row.lastFollowUpCount >= 2 &&
    deliveredOrDeliverable &&
    isIncomplete
  ) {
    priority = "High";
    ...
    const hasLogisticsException =
      includesAny(statusText, ["logistics", "物流异常"]) ||
      includesAny(row.notes, [
        "logistics",
        "物流异常",
        "delayed",
        "stuck",
    ...
    triggerReason = isInvitedOnly(row)
      ? "初次邀约后可轻跟进，但不应高于已寄样达人。"
      : "已联系但暂无回复，今天可跟进确认下一步。";
...
export function buildSummary(tasks: Task[]): Summary {
  return {
    ...
    failedWarnings: tasks.filter((task) => task.failedWarnings.length > 0)
      .length,
  };
}
