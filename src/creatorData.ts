...
export const CREATOR_TEMPLATE_COLUMNS: Array<{
  header: string;
...
function sameStore(a: CreatorRow, b: CreatorRow): boolean {
  return (
    normalizeStoreId(a.storeId, a.storeName) ===
    normalizeStoreId(b.storeId, b.storeName)
  );
}

export function creatorIdentityKeys(
  row: Pick<CreatorRow, "username" | "profileLink">,
): string[] {
  return [
...
export function isSameCreator(
  a: Pick<CreatorRow, "username" | "profileLink">,
  b: Pick<CreatorRow, "username" | "profileLink">,
): boolean {
...
export function getDuplicateCheck(
  ...
  const matchingRows = rows.filter(
    (candidate) => candidate.id !== row.id && isSameCreator(row, candidate),
  );
  ...
  const crossStoreRows = matchingRows.filter(
    (candidate) => !sameStore(row, candidate),
  );
...
export function countActiveCreatorSamples(
  row: CreatorRow,
  rows: CreatorRow[],
): number {
  return rows.filter(
    (candidate) =>
      isSameCreator(row, candidate) &&
      normalizeStoreId(candidate.storeId, candidate.storeName) ===
        normalizeStoreId(row.storeId, row.storeName) &&
      !/completed|failed|lost|归档|失败|合作完成|已完成/i.test(
        `${candidate.currentStatus} ${candidate.trackingStatus ?? ""}`,
      ),
  ).length;
}

export function buildDuplicateImportSummary(
  incomingRows: CreatorRow[],
  existingRows: CreatorRow[] = [],
...
export function copyCreatorBaseFields(
  target: CreatorRow,
  source: CreatorRow,
): CreatorRow {
  return {
...
function normalizeFollowUpHistory(
  row: CreatorRow,
): CreatorRow["followUpHistory"] {
  if (!Array.isArray(row.followUpHistory)) return [];

  return row.followUpHistory.filter(
    (entry) =>
      entry &&
...
function toStoredRow(row: CreatorRow): CreatorRow {
  return {
    ...
    lastFollowUpCount: Number.isFinite(Number(row.lastFollowUpCount))
      ? Number(row.lastFollowUpCount)
      : 0,
...
export function loadCreatorRows(): CreatorRow[] {
  ...
  try {
    ...
    return parsed
      .map((item) => toStoredRow(normalizeCreatorRowStore(item as CreatorRow)))
...
export function createBlankCreatorRow(
  productName = "",
  requiredVideos = 1,
...
export function saveCreatorRows(rows: CreatorRow[]): void {
  ...
  storage.setItem(
    ...
    JSON.stringify(rows.map(toStoredRow)),
  );
}
...
export function updateCreatorField(
  ...
  if (field === "lastFollowUpCount") {
    ...
    return {
      ...row,
      lastFollowUpCount: Number.isNaN(parsed) ? 0 : Math.max(0, parsed),
    };
  }
  ...
  if (field === "videoProgress") {
    ...
    const normalized =
      progressResult.warning && !progressResult.isOverRequired
        ? rawValue
        : progressResult.normalized;
    ...
    const statusPatch =
      typeof postedCount !== "number"
        ...
        : postedCount >= requiredVideos
          ...
          : postedCount > 0
            ? {
                currentStatus: `已发布 ${postedCount} 条 / 待补第 ${postedCount + 1} 条`,
                trackingStatus: "已发布部分视频",
              }
...
export function creatorRowsToCsv(rows: CreatorRow[]): string {
  ...
  const body = rows.map((row) => {
    ...
    return exportColumns
      .map((column) => {
        if (column.key === "isMultiSample")
          return escapeCsvValue(activeSampleCount > 1 ? "是" : "否");
        if (column.key === "multiSampleCount")
          return escapeCsvValue(activeSampleCount);
...
export function downloadCreatorRowsCsv(rows: CreatorRow[]): void {
  ...
  const blob = new Blob([creatorRowsToCsv(rows)], {
